...
class fab_client {
 ...
 protected:
  ...
  pthread_mutex_t fab_client_mutex;
  void primary_failure();
  bool init_members();
 public:
  fab_client(std::string dst);
  fab_protocol::status invoke(int proc, std::string req, std::string &rep);
...
fab_client::call_m(unsigned int proc, marshall &req, R &r)
{
	std::string rep;
        std::string res;
	int intret = invoke(proc, req.str(), rep);
        VERIFY( intret == fab_client_protocol::OK );
        unmarshall u(rep);
	u >> intret;
	if (intret < 0) return intret;
        u >> res;
        if (!u.okdone()) {
                fprintf(stderr, "fab_client::call_m: failed to unmarshall the reply.\n"
                       "You probably forgot to set the reply string in "
                       "fab::client_invoke, or you may call RPC 0x%x with wrong return "
                       "type\n", proc);
                VERIFY(0);
...
template<class R, class A1> int
  fab_client::call(unsigned int proc, const A1 & a1, R & r)
{
  marshall m;
  m << a1;
...
template<class R, class A1, class A2> int
  fab_client::call(unsigned int proc, const A1 & a1, const A2 & a2, R & r)
{
  marshall m;
  m << a1;
  m << a2;
...
template<class R, class A1, class A2, class A3> int
  fab_client::call(unsigned int proc, const A1 & a1, 
